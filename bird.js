...
//Metodo que optiene todas tgggglas aves
router.get('/', function (req, res) {
   Bird.find({}, (err, birds) => {
       if(err) {
           return res.status(500).json({ message: 'Error obteniendo avesffvfvvfvfvfv', err });
       }
       if(birds.length === 0){
           return res.status(404).json({ message: 'La lista está vacsfvfía' });
       }
       res.json({ status_code: 200, status_message: 'Ofsvfvk', data: birds });
   });
});
